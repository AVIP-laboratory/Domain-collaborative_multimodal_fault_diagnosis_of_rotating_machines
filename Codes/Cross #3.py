...
if job == 1:
    ...
    layer_A = TimeDistributed(Conv2D(16, (3, 3), padding="same", activation='relu'))(Input_time)
    layer_A = TimeDistributed(Conv2D(16, (2, 2), padding="same", activation='relu', strides=2))(layer_A)
    SKIP_A1 = TimeDistributed(Conv2D(32, (1, 1), padding="same"))(layer_A)
    ...
    SKIP_A2 = TimeDistributed(Conv2D(64, (1, 1), padding="same"))(layer_A)

    layer_A = TimeDistributed(Conv2D(64, (3, 3), padding="same"))(layer_A)
    CONNECTION_A2 = SKIP_A2 + layer_A
    layer_A = TimeDistributed(ReLU())(CONNECTION_A2)
    layer_A = TimeDistributed(Conv2D(64, (2, 2), padding="same", activation='relu', strides=2))(layer_A)

    layer_A = TimeDistributed(GlobalAveragePooling2D())(layer_A)
    layer_A = LSTM(64, return_sequences=True)(layer_A)

    ## FFT-ConvBlock1 & BiLSTM--------------------------------------------------------------------------------------
    Input_FFT = Input(shape=(35, 20, 20, 4))
    layer_B = TimeDistributed(Conv2D(16, (3, 3), padding="same", activation='relu'))(Input_FFT)
    layer_B = TimeDistributed(Conv2D(16, (2, 2), padding="same", activation='relu', strides=2))(layer_B)
    SKIP_B1 = TimeDistributed(Conv2D(32, (1, 1), padding="same"))(layer_B)
    ...
    SKIP_B2 = TimeDistributed(Conv2D(64, (1, 1), padding="same"))(layer_B)

    layer_B = TimeDistributed(Conv2D(64, (3, 3), padding="same"))(layer_B)
    CONNECTION_B2 = SKIP_B2 + layer_B
    layer_B = TimeDistributed(ReLU())(CONNECTION_B2)
    layer_B = TimeDistributed(Conv2D(64, (2, 2), padding="same", activation='relu', strides=2))(layer_B)

    layer_B = TimeDistributed(GlobalAveragePooling2D())(layer_B)
    layer_B = LSTM(64, return_sequences=True)(layer_B)
    ...
    ## Time-ConvBlock2----------------------------------------------------------------------------------------------
    Time_A1 = Conv1D(64, 3, padding="same", strides=2)(FF_A3)
    ...
    ## Freq-ConvBlock2----------------------------------------------------------------------------------------------
    Freq_B1 = Conv1D(64, 3, padding="same", strides=2)(FF_B3)
    ...
    TimeFreq_C1 = Conv1D(64, 3, padding="same", strides=2)(FF_C3)
    ...
    ## Dense--------------------------------------------------------------------------------------------------------
    Con = Concatenate()([Time_A_GAP, Freq_B_GAP, TimeFreq_C_GAP])
    Con = Dense(32)(Con)
